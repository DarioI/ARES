...
class TreeWindow:
    ...
    def __init__(self,parent,treewidget):

        self.treewidget = treewidget
        self.mainwin = parent
        self.treewidget.itemDoubleClicked.connect(self.itemDoubleClickedHandler)
        self.createActions()
        ...
        for c in sorted(classes, key=lambda c: c.name):
            ...
            for path in sig.class_path:
                if path not in path_node[0]:
                    path_node[0][path] = ({}, QtGui.QTreeWidgetItem(str(path_node[1])))
